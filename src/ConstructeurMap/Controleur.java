		...
		try (BufferedReader lecture = new BufferedReader(new FileReader("ConstructeurMap/mines.txt")))
		{
			...
			while ((line = lecture.readLine()) != null)
			{
				...
				if (minesSection)
				{
					...
					Region region = Controleur.getRegionChar0(charRegion);

					Mine mine = new Mine(Integer.parseInt(parts[0].trim()), Integer.parseInt(parts[1].trim()),
					...
					{
						System.out.println(mines.getNumMine() + " " + mines.getX() + " " + mines.getY() + " " + region);
					}
				...
				else if (routesSection)
				{
					...
					System.out.println("-------------------");
					System.out.println("Les Routes entre mine : " + mineDep.getNumMine() + " et " + " mine "
							+ mineArr.getNumMine());
					System.out.println("-------------------");
					for (Routes routes : this.lstRoutes)
					{
						System.out.println(routes.getNbTroncon() + "  " + routes.getMineDep().getNumMine() + "  "
								+ routes.getMineArriv().getNumMine());
					}
				}
			}
		} catch (Exception e)
